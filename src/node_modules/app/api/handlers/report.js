import {find, map, forEach, isUndefined, range, indexOf, wait} from "app/core/utils"
...
function getCategoryData(report, category, locationType, year, codes) {
  // TODO Use report.version to invalidate cache
  return fromCache(`categoryData/${report.id}/${category.id}/${locationType}/${year}`, TTL_SHORT, key => {
  ...
  return Promise.all(map(categoriesByLocationType, (categories, locationType) => {
    ...
    .then(codes => {
      return Promise.all(categories.map(category => {
        return getCategoryData(category, locationType, year, codes)
        .then(valueByCode => {
...
function getReportViewDataForMap(request, report, view) {
  ...
  return getRecordsByLocationType(year, categoriesByLocationType)
  .then(recordsByLocationType => {
  ...
  .then(geos => {
    ...
    categories.forEach(categoryId => {
      ...
      range(startYear, endYear + 1).forEach(year => {
        promises.push(getCategoryData(category, locationType, year, codes)
        .then(valueByCode => {
...
export function getViewRecordsInTimeRange(report, view, startYear, endYear) {
...
export function getViewDataForTrends(request, report, view) {
  const endYear = 2014
  const startYear = endYear - +view.timeRange.substring(4) + 1
...
export function getViewData(request) {
  return findReportById(+request.params.id)
  .then(report => {
    const view = find(report.config.views, "id", +request.params.viewId)
    if (view.type === "trends") {
      return getViewDataForTrends(request, report, view)
    }
  })
}
