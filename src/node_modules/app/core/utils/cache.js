...
export const del = promisify("del")

export function fromCache(key, time, request, before, after) {
  return get(key)
  .then(response => {
    if (response) {
    ...
    let promise = Promise.resolve(request(key))
    if (before) {
    ...
    .then(value => {
      ...
      return setex(key, timeToCache, JSON.stringify({value}))
      .then(() => after ? after(value) : value)
    })
  })
}
