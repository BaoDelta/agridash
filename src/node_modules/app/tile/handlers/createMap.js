...
import mockLayers from "app/tile/handlers/mockLayers"
...
function getMapOptions(request) {
  const key = request.params.id + request.url.search
  ...
  if (!mapOptions) {
    ...
    mockLayers.forEach((mockLayer, index) => {
      if (hiddenIndex[index]) {
        return;
      }
      const layerName = `${mockLayer.type}${index}`
      const layerOptions = layerByType[mockLayer.type]({
        ...mockLayer,
        styleName: layerName
      }, request.query)
