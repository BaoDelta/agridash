  ...
  Alpaca(element, {
    ...
    options: {
        ...
        buttons: {
          submit: {
            ...
            click() {
              ...
              if (this.isValid()) {
                dispatch(submitForm(elements.form.id, this.getValue()))
                .then(() => {
                  const {form: {[elements.form.id]: newFormState}} = getState()
                  if (newFormState.result && formOptions.done) {
                    formOptions.done(newFormState.result)
                  }
                })
              } else {
      ...
      postRender(done) {
        ...
        elements.loadError = $(`<span class="label label-danger"></span>`)
        .css({
          marginRight: "1rem",
          fontSize: "110%"
        })
