  ...
  return {
    ...
    fields: {
      ...
      locationType: getLocationTypeOptions({
        ...
        events: {
          change() {
            const locationType = this.getValue()
            if (locationType) {
              const locations = this.parent.childrenByPropertyId.locations
              locations.showLoading();
              store.dispatch(fetchGeos(locationType)).then(() => {
                const {geo: geoState} = store.getState()
                locations.options.dataSource = geoState[locationType].geos.map(geo => ({
                  value: geo.code,
                  text: geo.name
                }))
                locations.refresh()
                locations.hideLoading()
              })
            }
          }
        }
