...
class MapView extends Component {
  ...
  componentDidMount() {
    ...
    const {tileUrl, sessionId} = clientConfig
    ...
    if (combine) {
      this.tileLayer = L.tileLayer(`${tileUrl}/report/${report.id}/view/${view.id}/{z}/{x}/{y}.png`
        + `?sessionId=${sessionId}&resultId=12014`).addTo(this.map)
    } else {
      L.tileLayer(`${tileUrl}/${sessionId}/12014/{z}/{x}/{y}.png?h=1,2,3,4`).addTo(this.map)
      L.tileLayer(`${tileUrl}/${sessionId}/12014/{z}/{x}/{y}.png?h=0,2,3,4`).addTo(this.map)
      L.tileLayer(`${tileUrl}/${sessionId}/12014/{z}/{x}/{y}.png?h=0,1,3,4`).addTo(this.map)
      L.tileLayer(`${tileUrl}/${sessionId}/12014/{z}/{x}/{y}.png?h=0,1,2,4`).addTo(this.map)
      L.tileLayer(`${tileUrl}/${sessionId}/12014/{z}/{x}/{y}.png?h=0,1,2,3`).addTo(this.map)
    }
  }
  ...
  componentDidUpdate() {
    ...
    this.tileLayer.setUrl(`${tileUrl}/report/${report.id}/view/${view.id}/{z}/{x}/{y}.png`
      + `?sessionId=${sessionId}&resultId=12014`)
  }
